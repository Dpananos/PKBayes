...
map_draw_location = here('analysis', 'data', 'map_parameter_draws.RDS')
map_draws = readRDS(map_draw_location)

mcmc_draw_location =  here('analysis', 'data', 'mcmc_parameter_draws.RDS')
...
```{r}
#Save dose recommendations
dose_location = here("analysis", "data","experiment_2_doses.csv")
doses_for_max %>% 
  select(patient, p, mcmc_estimated_dose, map_estimated_dose) %>% 
  write_csv(dose_location)


dose_location = here("analysis", "data","experiment_1_doses.csv")
doses_for_12%>% 
  select(patient, p, mcmc_estimated_dose, map_estimated_dose) %>% 
  write_csv(dose_location)
...
true_pk_param_location = here("analysis","data","simulated_data.csv")
true_pk_params = read_csv(true_pk_param_location)

experiment_1_doses = read_csv(here("analysis", "data","experiment_1_doses.csv")) %>% 
  filter(p<=0.5)
experiment_2_doses = read_csv(here("analysis", "data","experiment_2_doses.csv")) %>% 
  filter(p<=0.5)
...
calibration_1 = experiment_1_doses %>% 
...
calibration_2 = experiment_2_doses %>% 
  left_join(true_pk_params, by = c('patient' = 'subjectids')) %>% 
