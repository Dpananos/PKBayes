...
fit = sampling(model,               readRDS('../data/simulated_data_dump.Rdump'),
               warmup=1000,
               iter=4000,
               chains = 12,
               thin = 2,
...
subject_params = fit %>% 
  gather_draws(Cl[subject], ke[subject], ka[subject], alpha[subject], t[subject], n = 2000) %>% 
  mean_qi()


write_csv(subject_params,'../data/hmc_patient_parameter_estimates.csv')
```
