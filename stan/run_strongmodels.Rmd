...
no_condition %>% 
  bind_cols(predictions) %>% 
  filter(subjectids<13) %>% 
  mutate(mcmc_width = mcmc_high - mcmc_low,
         map_width = map_high - map_low) %>% 
  select(times, subjectids, map_width, mcmc_width) %>% 
  gather(estimator, width, -times, -subjectids) %>% 
  ggplot(aes(times, width, color = estimator))+
  geom_line()+
