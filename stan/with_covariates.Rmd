...
model{
  mu_CL ~ lognormal(0.26,0.04);
  beta_CL[1] ~ normal(0.5, 0.07);
  beta_CL[2] ~ normal(0.18, 0.04);
  s_CL ~ lognormal(-1.64,0.12);
  z_CL ~ normal(0,1);
  
  mu_t ~ normal(0.97, 0.05);
  s_t ~ lognormal(-1.41, 0.12);
  z_t ~ normal(0,1);
  
  alpha ~ beta(2,2);
  sigma ~ lognormal(-1.76, 0.06);
  yobs ~ lognormal(log(Cpred), sigma);
}
...
generated quantities{
  ...
  real beta_1_cl = normal_rng(0.5, 0.07);
  real beta_2_cl = normal_rng(0.16, 0.04);
  vector[2] beta_CL = [beta_1_cl, beta_2_cl]';
  real s_CL = lognormal_rng(-1.64, 0.12);
  ...
  real s_t = lognormal_rng(-1.41, 0.12);
